const nodemailer = require("nodemailer");

// Create a test account or replace with real credentials.
const transporter = nodemailer.createTransport({
  host: "smtp.gmail.com",
  port: 587,
  secure: false, // true for 465, false for other ports
  auth: {
    user: "ayub56258@gmail.com",
    pass: "xkwqqsblhunbblmu",
  },
});

// Wrap in an async IIFE so we can use await.
(async () => {
  const info = await transporter.sendMail({
    from: '"ayub56258@gmail.com',
    to: "studentsmit782@gmail.com",
    subject: "Hello ✔",
    // text: "Hello world?", // plain‑text body
    html: `<!DOCTYPE html>
<html>
<head>
  <title>SMIT ID Card</title>
</head>
<body style="font-family: Arial, sans-serif; background-color: #f2f2f2; padding: 20px;">

  <div style="display: flex; justify-content: center; gap: 40px;">

    <!-- Front Side -->
    <div style="width: 260px; border: 2px solid #0074b7; background: #fff; border-radius: 8px; padding: 20px; text-align: center; position: relative;">
      <div style="border-top: 6px solid #1ca261; border-bottom: 6px solid #1ca261; padding: 10px;">
        <!-- <h2 style="color: #0074b7; margin: 5px 0;">SM<span style="color: #28a745;">I</span>T</h2> -->
         <img src="data:image/png;base64," height="100px">
        <div style="background-color: #0074b7; color: white; font-size: 12px; padding: 3px 6px; border-radius: 4px; display: inline-block;">
          SAYLANI MASS IT TRAINING PROGRAM
        </div>
        <div style="margin-top: 10px;">
          <img src="https://static.vecteezy.com/system/resources/thumbnails/005/346/410/small_2x/close-up-portrait-of-smiling-handsome-young-caucasian-man-face-looking-at-camera-on-isolated-light-gray-studio-background-photo.jpg" alt="Student Photo" style="border: 2px solid #0074b7; border-radius: 5px; height: 120px;">
        </div>
        <h3 style="margin: 10px 0 4px 0;">AYUB AHMED</h3>
        <p style="margin: 0; font-size: 13px;">AI-CHATBOT</p>
        <p style="margin: 2px 0; font-weight: bold; color: #0074b7;">GD-83236</p>
      </div>
    </div>

    <!-- Back Side -->
    <div style="width: 260px; border: 2px solid #0074b7; background: #fff; border-radius: 8px; padding: 20px;">
      <div style="border-top: 6px solid #1ca261; border-bottom: 6px solid #1ca261; padding: 10px;">
        <p style="margin: 5px 0;"><strong>Name:</strong> Ayub Ahmed</p>
        <p style="margin: 5px 0;"><strong>Father name:</strong> Muhammad Iqbal</p>
        <p style="margin: 5px 0;"><strong>CNIC:</strong> 4210195780892</p>
        <p style="margin: 5px 0;"><strong>Course:</strong> AI BATCH (8)</p>

        <div style="text-align: center; margin: 10px 0;">
          <img src="https://api.qrserver.com/v1/create-qr-code/?data=GD-83236&size=100x100" alt="QR Code">
        </div>

        <p style="font-size: 11px; text-align: center; margin-top: 10px;">
          <strong>Note:</strong> This card is for SMIT’s premises only. If found please return to SMIT.
        </p>

        <div style="text-align: center; margin-top: 20px;">
          <div style="width: 100px; border-top: 1px solid #000; margin: 0 auto; font-size: 12px;">Issuing authority</div>
        </div>
      </div>
    </div>

  </div>

</body>
</html>
`, // HTML body
  });

  console.log("Message sent:", info.messageId);
})();